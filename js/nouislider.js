...
jQuery(document).ready(function($){
    	// SIZE SLIDER
    	var sliderFormat = document.getElementById('size-slider');
    	var inputFormat = document.getElementById('size');
    	...
    	sliderFormat.noUiSlider.on('update', function( values, handle ) {
    		...
    		inputFormat.value = Math.round(value);
    	});
    
    	inputFormat.addEventListener('change', function(){
    		//sliderFormat.noUiSlider.set(this.value); // uncomment to allow field to adjust slider
    	});
    	// SIZE SLIDER END
    
    	// RATE SLIDER
    	var sliderFormat2 = document.getElementById('rate-slider');
    	var inputFormat2 = document.getElementById('rate');
    	...
    	sliderFormat2.noUiSlider.on('update', function( values, handle ) {
    		var value = values[handle];
    		inputFormat2.value = Math.round(value);
    	});
    
    	inputFormat2.addEventListener('change', function(){
    		//sliderFormat2.noUiSlider.set(this.value); // uncomment to allow field to adjust slider
    	});
    	// RATE SLIDER END
    	...
    	$(sizeselect).change(function() {
    	    ...
    	    $('#size-increment').html(val);
    	    calcTransferTime();
    	});
    	var sizeval = $('option:selected',sizeselect).text();
    	$('#size-increment').html(sizeval);
    
    	var rateselect = document.getElementById('rate-select');
    	$(rateselect).change(function() {
    		val = $('option:selected',this).text();
    	    $('#rate-increment').html(val);
    	    calcTransferTime();
    	});
    	var rateval = $('option:selected',rateselect).text();
    	$('#rate-increment').html(rateval);
    	...
    	rateslider.noUiSlider.on('update', function(){
    		calcTransferTime();
    	});
	});
	function evenNav() {
    var e = -1;
    $(".evennavjs").css("height", ""),
        $(".evennavjs").each(function () {
...
function setCtaWidth() {
    ...
    if (e > 1099) {
        var t = e / 2 + 14;
        $(".fifty-fifty-ctas a:nth-child(2)").width(t);
    } else $(".fifty-fifty-ctas a:nth-child(2)").width("");
}
function parallaxMagic() {
    var e = new ScrollMagic.Controller();
    if ($("#parallax").length) {
        var t = new ScrollMagic.Scene({ triggerElement: "#parallax", triggerHook: "onEnter", duration: "200%" }).setTween("#parallax > div", { y: "90%", ease: Linear.easeNone });
        e.addScene(t);
    }
}
function statMagic() {
    var e = ($(window).height(), $("#statjs").outerHeight(), new ScrollMagic.Controller()),
        t = new ScrollMagic.Scene({ triggerElement: "#statjs", triggerHook: 1 }).on("start", function () {
            $("#statjs").hasClass("activated") ||
                ($("#statjs .counter").each(function () {
                    var e = $(this),
                        t = e.attr("data-count");
                ...
                $("#statjs").addClass("activated"));
        });
    e.addScene(t);
}
function calcTransferTime() {
    var e = jQuery("#size-select").val(),
        t = jQuery("input[id=size]").val(),
        a = jQuery("#rate-select").val(),
        n = jQuery("input[id=rate]").val();
    "KB" == e ? (bits = 8e3 * t) : "MB" == e ? (bits = 8e6 * t) : "GB" == e ? (bits = 8e9 * t) : "TB" == e && (bits = 8e12 * t),
        "Kbps" == a ? (bps = 1e3 * n) : "Mbps" == a ? (bps = 1e6 * n) : "Gbps" == a && (bps = 1e9 * n),
        (rawseconds = Math.round(bits / bps)),
        (totaldays = Math.floor(rawseconds / 86400)),
        (days = totaldays),
        (totalhours = Math.floor(rawseconds / 3600)),
        (hours = totalhours % 24),
        (totalminutes = Math.floor(rawseconds / 60)),
        (minutes = totalminutes % 60),
        (totalseconds = rawseconds),
        (seconds = totalseconds % 60),
        1 === days ? (dtext = "day") : (dtext = "days"),
        1 === hours ? (htext = "hour") : (htext = "hours"),
        1 === minutes ? (mtext = "minute") : (mtext = "minutes"),
        1 === seconds ? (stext = "second") : (stext = "seconds"),
        jQuery("#transfer-time").html(days + " " + dtext + ", " + hours + " " + htext + ", " + minutes + " " + mtext + ", " + seconds + " " + stext);
}